...
const Edit = () => {
  ...
  const { setIsCreateDepartmentType } = useDepartmentContext();
  ...
  const onHandleSubmit = async (e) => {
    ...
    const res = await useEdit(
      ...
      `${import.meta.env.VITE_BASE_URL_API}/deartamenttypecontroller/updatedepartamentType`,
      `${import.meta.env.VITE_BASE_URL_API}/deartamenttypecontroller/updatedepartamentTypetranslation`,
      [
        { language_id },
      ...
      ["status_id"],
      `${import.meta.env.VITE_BASE_URL_API}/deartamenttypecontroller/createdepartamentTypetranslation`,
      [{ language_id }, { departament_type_id: id }],
      ["status_id"]
    );

    if (res.status === 200) {
      setIsCreateDepartmentType({ name: "Edit" });
      setIsCreate(false);
    }
  };
  ...
  return (
    <Wrapper title="Edit Department Type">
